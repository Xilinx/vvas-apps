...
/*
 * This file is using some of the work from Erik Walthinsen and
 * David Schleef and the original license for that work has been
...
vvas_xcompositor_allocate_internal_buffers (GstVvasXCompositor * self)
{
  ...
  for (chan_id = 0; chan_id < self->num_request_pads; chan_id++) {
    iret =
        ...
        VVAS_BO_DEVICE_RAM, self->in_mem_bank, &priv->Hcoff[chan_id]);
    if (iret < 0) {
      GST_ERROR_OBJECT (self,
    ...
    iret =
        vvas_xrt_alloc_xrt_buffer (priv->dev_handle, COEFF_SIZE,
        VVAS_BO_DEVICE_RAM, self->in_mem_bank, &priv->Vcoff[chan_id]);
    if (iret < 0) {
      GST_ERROR_OBJECT (self,
    ...
    iret =
        ...
        VVAS_BO_DEVICE_RAM, self->in_mem_bank, &priv->msPtr[chan_id]);
    if (iret < 0) {
      GST_ERROR_OBJECT (self,
          "failed to allocate vertical coefficients command buffer..");
      goto error;
...
vvas_xcompositor_open (GstVvasXCompositor * self)
{
  ...
  self->priv->cu_index =
      vvas_xrt_ip_name2_index (priv->dev_handle, self->kern_name);
  ...
  self->priv->ert_cmd_buf = (xrt_buffer *) calloc (1, sizeof (xrt_buffer));
  if (self->priv->ert_cmd_buf == NULL) {
  ...
  iret =
      vvas_xrt_alloc_xrt_buffer (self->priv->dev_handle, ERT_CMD_SIZE,
      XCL_BO_SHARED_VIRTUAL, XCL_BO_FLAGS_EXECBUF, self->priv->ert_cmd_buf);
  if (iret < 0) {
...
error:
  ...
  for (chan_id = 0; chan_id < self->num_request_pads; chan_id++) {
    vvas_xrt_free_xrt_buffer (self->priv->dev_handle,
        &self->priv->Hcoff[chan_id]);
    vvas_xrt_free_xrt_buffer (self->priv->dev_handle,
        &self->priv->Vcoff[chan_id]);
  }
    ...
    GstVvasXCompositorPad * sinkpad, GstBuffer ** inbuf)
{
  ...
  if (gst_is_vvas_memory (in_mem)
    ...
    GST_DEBUG_OBJECT (self, "received dma fd %d and its xrt BO = %u",
        dma_fd, bo);
...
static void
vvas_xcompositor_reg_write (GstVvasXCompositor * self, void *src, size_t size,
    size_t offset)
{
...
vvas_xcompositor_process (GstVvasXCompositor * self)
{
  ...
  vvas_xcompositor_reg_write (self, &value, sizeof (value),
      (XV_MULTI_SCALER_CTRL_ADDR_HWREG_NUM_OUTS_DATA + payload_offset));
  desc_addr = self->priv->msPtr[0].phy_addr;
  vvas_xcompositor_reg_write (self, &desc_addr, sizeof (desc_addr),
  ...
  if (self->priv->cu_index > 31) {
    ert_cmd->cu_mask = 0;
    if (self->priv->cu_index > 63) {
      ert_cmd->data[0] = 0;
      if (self->priv->cu_index > 96) {
        ert_cmd->data[1] = 0;
        ert_cmd->data[2] = (1 << (self->priv->cu_index - 96));
      } else {
        ert_cmd->data[1] = (1 << (self->priv->cu_index - 64));
        ert_cmd->data[2] = 0;
      }
    } else {
      ert_cmd->data[0] = (1 << (self->priv->cu_index - 32));
    }
  } else {
    ert_cmd->cu_mask = (1 << self->priv->cu_index);
    ert_cmd->data[0] = 0;
    ert_cmd->data[1] = 0;
    ert_cmd->data[2] = 0;
  }
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    vvas_xcompositor,
    "Xilinx VVAS Compositor plugin", vvas_xcompositor_init,
    "0.1", "LGPL", "Xilinx VVAS SDK plugin", "http://xilinx.com/")
